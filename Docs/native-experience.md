...
## Compact player appearance

MPV now presents a small title row, glass play/pause and Settings controls, and a slim focusable timeline over a full-width bottom fade. Down from the buttons enters the timeline; Left/Right seek, Select toggles playback, and another Down retains the existing contextual action. Native AVKit keeps its system transport, hiding it only while settings are open and restoring it on dismissal. Engine selection, format support, playback callbacks, and diagnostic throttling are unchanged.

The shared settings panel floats above the bottom edge, uses a dark backing under Liquid Glass, and sizes to each tab's content instead of reserving a fixed large rectangle. Audio, subtitles, playback, episodes, sources, and details have capped scrolling regions so long lists remain accessible. Tab widths accommodate their actual column count. All supported controls remain available, including subtitle/audio timing, source and episode selection, output routing, and live channel actions.

Validated on the tvOS 27 simulator: native and MPV audio/subtitle selection and speed changes; live channel switching and return to Guide; native and MPV play/pause and seeking after closing settings; MPV seeking after controls hide; twenty-track audio/subtitle lists; and subtitle timing Reset keeping focus visible. Transport geometry checks enforce the compact two-row layout. Screenshots were reviewed over bright synthetic video. The final Apple TV hardware build succeeds and passes strict deep code-signature verification. Physical-device playback and performance testing remain outstanding.
