...
Home scroll regression: the pinned hero sits outside the shelves' scroll view. Returning focus to that hero must settle the shelves to `home_top` once, in a nonanimated transaction; otherwise the native bar retains the old scroll translation and stays offscreen (reproduced at y=-445). Do not use delayed scroll retries or continuously enforce the top position, which can fight the next Down. Regression coverage must use the real Home screen: walk three, five and eight steps down, return to the native tabs, enter the shelves again and switch to Search. A generic tab fixture or Search-only test does not cover this geometry.
